...
public:
    ...
    void setMouseSensitivity(float degreesPerPixel) { m_mouseSensitivityDegPerPixel = degreesPerPixel; }
    float mouseSensitivity() const { return m_mouseSensitivityDegPerPixel; }

    void addMouseDeltaPixels(float dx, float dy);
    void moveLocal(const glm::vec3& localDir, float deltaSeconds, float speedMultiplier = 1.0f);
