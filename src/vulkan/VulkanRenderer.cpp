...
uint32_t findMemoryTypeIndex(VkPhysicalDevice physicalDevice, uint32_t typeFilter, VkMemoryPropertyFlags props)
{
    VkPhysicalDeviceMemoryProperties memProps{};
    vkGetPhysicalDeviceMemoryProperties(physicalDevice, &memProps);

    for (uint32_t i = 0; i < memProps.memoryTypeCount; ++i) {
        if ((typeFilter & (1u << i)) != 0 && (memProps.memoryTypes[i].propertyFlags & props) == props) {
            return i;
        }
    }
    return UINT32_MAX;
...
bool VulkanRenderer::init(const InitInfo& info, std::string& outError)
{
    ...
    if (!createInstance(outError) || !createSurface(info, outError) || !pickPhysicalDevice(outError)
        || !createDevice(outError) || !createDescriptorSetLayout(outError) || !createCommandPool(outError)
    ...
    if (!createSwapchain(width, height, outError) || !createCommandBuffers(outError) || !createSyncObjects(outError)) {
        cleanup();
        return false;
    }
...
void VulkanRenderer::drawFrame()
{
    ...
    if (acquire == VK_ERROR_OUT_OF_DATE_KHR) {
        std::string error;
        (void)recreateSwapchain(m_swapchainExtent.width, m_swapchainExtent.height, error);
        return;
    }
    ...
    if (!recordCommandBuffer(m_commandBuffers[m_currentFrame], imageIndex, recordError)) {
        return;
    }
...
void VulkanRenderer::setCamera(const vv::core::Camera& camera, float timeSeconds)
{
    m_camera = camera;
    m_timeSeconds = timeSeconds;
}

void VulkanRenderer::setWorldConfig(const glm::uvec3& chunkSizeVoxels, const glm::vec3& voxelSize)
{
    if (m_initialized) {
        return;
    }
...
bool VulkanRenderer::createInstance(std::string& outError)
{
    ...
    const uint32_t requestedApiVersion = std::min(loaderVersion, VK_API_VERSION_1_4);

    VkApplicationInfo appInfo{};
    appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
    appInfo.pApplicationName = "Vulkanic Pixels";
    appInfo.applicationVersion = VK_MAKE_VERSION(0, 1, 0);
    appInfo.pEngineName = "VulkanicPixels";
...
bool VulkanRenderer::createSurface(const InitInfo& info, std::string& outError)
{
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to create Win32 Vulkan surface (" + vkResultToString(r) + ").";
        return false;
    }
    return true;
}
...
bool VulkanRenderer::pickPhysicalDevice(std::string& outError)
{
    ...
    if (deviceCount == 0) {
        outError = "No Vulkan-capable GPU found (vkEnumeratePhysicalDevices returned 0).";
        return false;
    }
    ...
    outError = "No suitable Vulkan device found (requires graphics + present queue and VK_KHR_swapchain).";
    return false;
}
...
VkSurfaceFormatKHR VulkanRenderer::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats)
{
    ...
    // Fall back to common formats; we'll later fail with a clear error if we can't run compute on them.
    ...
    uint32_t height)
{
    ...
    VkExtent2D actual{};
    actual.width = clampU32(width, capabilities.minImageExtent.width, capabilities.maxImageExtent.width);
    actual.height = clampU32(height, capabilities.minImageExtent.height, capabilities.maxImageExtent.height);
    return actual;
}

bool VulkanRenderer::createSwapchain(uint32_t width, uint32_t height, std::string& outError)
{
    ...
    const VkExtent2D extent = chooseSwapExtent(support.capabilities, width, height);
    ...
    if (m_swapchainFormat != VK_FORMAT_B8G8R8A8_UNORM && m_swapchainFormat != VK_FORMAT_B8G8R8A8_SRGB
        && m_swapchainFormat != VK_FORMAT_R8G8B8A8_UNORM && m_swapchainFormat != VK_FORMAT_R8G8B8A8_SRGB) {
        outError = "Unsupported swapchain format for voxel renderer (expected BGRA8/RGBA8 UNORM or SRGB).";
        return false;
    }
    ...
    for (size_t i = 0; i < m_swapchainImages.size(); ++i) {
        ...
        r = vkCreateImageView(m_device, &viewInfo, nullptr, &m_swapchainImageViews[i]);
        if (r != VK_SUCCESS) {
    ...
    if (!createStorageResources(outError) || !createDescriptorSet(outError) || !createComputePipeline(outError)) {
        return false;
    }
...
bool VulkanRenderer::recreateSwapchain(uint32_t width, uint32_t height, std::string& outError)
{
    if (width == 0 || height == 0) {
        return true;
    }
...
bool VulkanRenderer::createDescriptorSetLayout(std::string& outError)
{
    ...
    VkDescriptorSetLayoutBinding bindings[] = {voxelBufferBinding, outputBufferBinding, sceneBinding};
    ...
    VkResult r = vkCreateDescriptorSetLayout(m_device, &info, nullptr, &m_descriptorSetLayout);
    if (r != VK_SUCCESS) {
        outError = "Failed to create descriptor set layout (" + vkResultToString(r) + ").";
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to create pipeline layout (" + vkResultToString(r) + ").";
        return false;
    }
...
VkShaderModule VulkanRenderer::createShaderModule(const std::vector<char>& code, std::string& outError)
{
...
{
    vv::voxel::World world(vv::voxel::Extent3u{m_chunkSizeVoxels.x, m_chunkSizeVoxels.y, m_chunkSizeVoxels.z});
    const vv::voxel::Chunk& chunk = world.chunk0();

    const auto& voxels = chunk.rawVoxelsU32();
    const VkDeviceSize voxelBytes = static_cast<VkDeviceSize>(voxels.size() * sizeof(uint32_t));
    if (voxelBytes == 0) {
    ...
    voxelBuf.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    ...
    uint32_t voxelMemType = findMemoryTypeIndex(m_physicalDevice, voxelReq.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate voxel buffer memory (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to bind voxel buffer memory (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to create voxel staging buffer (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
    ...
    uint32_t stagingType = findMemoryTypeIndex(
        m_physicalDevice,
        stagingReq.memoryTypeBits,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    if (stagingType == UINT32_MAX) {
        outError = "No suitable host-visible memory type found for voxel staging buffer.";
        cleanupTemp();
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate voxel staging memory (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to bind voxel staging memory (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate voxel upload command buffer (" + vkResultToString(r) + ").";
        cleanupTemp();
        return false;
    }
...
bool VulkanRenderer::createSceneResources(std::string& outError)
{
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to create scene uniform buffer (" + vkResultToString(r) + ").";
        return false;
    }
    ...
    if (memType == UINT32_MAX) {
        outError = "No suitable host-visible memory type found for scene uniform buffer.";
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate scene uniform buffer memory (" + vkResultToString(r) + ").";
        return false;
    }

    r = vkBindBufferMemory(m_device, m_sceneBuffer, m_sceneBufferMemory, 0);
    if (r != VK_SUCCESS) {
        outError = "Failed to bind scene uniform buffer memory (" + vkResultToString(r) + ").";
        return false;
    }
...
{
    const uint64_t elements = static_cast<uint64_t>(m_swapchainExtent.width) * static_cast<uint64_t>(m_swapchainExtent.height);
    const VkDeviceSize bufferSize = static_cast<VkDeviceSize>(elements * sizeof(uint32_t));
    ...
    outBuf.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
    ...
    uint32_t outMemType = findMemoryTypeIndex(m_physicalDevice, outReq.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    if (outMemType == UINT32_MAX) {
        // Fallback: still works, just slower.
        outMemType = findMemoryTypeIndex(
            m_physicalDevice,
            outReq.memoryTypeBits,
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
    }
    if (outMemType == UINT32_MAX) {
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate output buffer memory (" + vkResultToString(r) + ").";
        return false;
    }

    r = vkBindBufferMemory(m_device, m_outputBuffer, m_outputBufferMemory, 0);
    if (r != VK_SUCCESS) {
        outError = "Failed to bind output buffer memory (" + vkResultToString(r) + ").";
        return false;
    }
...
bool VulkanRenderer::createDescriptorSet(std::string& outError)
{
    ...
    VkResult r = vkCreateDescriptorPool(m_device, &pool, nullptr, &m_descriptorPool);
    if (r != VK_SUCCESS) {
        outError = "Failed to create descriptor pool (" + vkResultToString(r) + ").";
        return false;
    }
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate descriptor set (" + vkResultToString(r) + ").";
        return false;
    }
...
bool VulkanRenderer::createComputePipeline(std::string& outError)
{
    ...
    VkResult r = vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipe, nullptr, &m_computePipeline);
    vkDestroyShaderModule(m_device, compModule, nullptr);
    if (r != VK_SUCCESS) {
        outError = "Failed to create compute pipeline (" + vkResultToString(r) + ").";
        return false;
    }
...
bool VulkanRenderer::createCommandPool(std::string& outError)
{
    ...
    VkResult r = vkCreateCommandPool(m_device, &poolInfo, nullptr, &m_commandPool);
    if (r != VK_SUCCESS) {
...
bool VulkanRenderer::createCommandBuffers(std::string& outError)
{
    ...
    if (r != VK_SUCCESS) {
        outError = "Failed to allocate command buffers (" + vkResultToString(r) + ").";
        return false;
    }
...
bool VulkanRenderer::recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex, std::string& outError)
{
    ...
    vkCmdPipelineBarrier(
        cmd,
        VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT | VK_PIPELINE_STAGE_TRANSFER_BIT | VK_PIPELINE_STAGE_HOST_BIT,
    ...
    const uint32_t isBgra = (m_swapchainFormat == VK_FORMAT_B8G8R8A8_UNORM || m_swapchainFormat == VK_FORMAT_B8G8R8A8_SRGB)
        ? 1u
        : 0u;
    PushConstants push{};
    push.screen = glm::uvec4(m_swapchainExtent.width, m_swapchainExtent.height, isBgra, m_frameCounter);
    ...
    vkCmdCopyBufferToImage(cmd, m_outputBuffer, m_swapchainImages[imageIndex], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
