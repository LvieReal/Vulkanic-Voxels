...
public:
    ...
    void setWorldConfig(const glm::uvec3& chunkSizeVoxels, const glm::vec3& voxelSize);
...
private:
    ...
    bool recreateSwapchain(uint32_t width, uint32_t height, std::string& outError);
    ...
    bool recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex, std::string& outError);
    ...
    VkShaderModule createShaderModule(const std::vector<char>& code, std::string& outError);
