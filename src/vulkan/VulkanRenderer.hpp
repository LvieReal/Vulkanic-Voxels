...
class VulkanRenderer final {
 ...
 private:
  ...
  // Pass 56: the shadow-ray jitter handed to the shader each frame
  // (VV_SHADOW_JITTER), in PIXEL FOOTPRINTS of origin displacement. < 0 =
  // unset, so the shader's kShadowJitterDefault applies; 0 = jitter off,
  // bit-identical to the un-jittered estimate.
