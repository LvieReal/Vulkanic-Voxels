    ...
    : QWidget(parent)
{
    ...
    connect(m_pauseMenu, &PauseMenu::backToGameRequested, this, [this]() { hidePauseMenu(); });
...
void VulkanWidget::showEvent(QShowEvent* event)
{
    ...
    m_pendingWidth = static_cast<uint32_t>(std::max(1, static_cast<int>(width() * dpr)));
    m_pendingHeight = static_cast<uint32_t>(std::max(1, static_cast<int>(height() * dpr)));
    ensureInitialized();
    lockMouse();
}
...
void VulkanWidget::resizeEvent(QResizeEvent* event)
{
    ...
    const qreal dpr = devicePixelRatioF();
    m_pendingWidth = static_cast<uint32_t>(std::max(1, static_cast<int>(event->size().width() * dpr)));
    m_pendingHeight = static_cast<uint32_t>(std::max(1, static_cast<int>(event->size().height() * dpr)));
    if (m_renderer) {
...
void VulkanWidget::ensureInitialized()
{
    ...
    if (!hwnd) {
        QMessageBox::critical(this, "Vulkan", "Failed to obtain a native window handle (HWND).");
        return;
    }
    ...
    if (!m_renderer->init(init, error)) {
        QMessageBox::critical(this, "Vulkan unsupported", QString::fromStdString(error));
        m_renderer.reset();
        return;
    }
    ...
    m_camera.setPosition(center + glm::vec3(0.0f, chunkSizeWorld.y * 0.35f, chunkSizeWorld.z * 1.75f));
...
void VulkanWidget::tick()
{
    ...
    m_renderer->setCamera(m_camera, static_cast<float>(m_gameTimer.totalSeconds()));
...
void VulkanWidget::mouseMoveEvent(QMouseEvent* event)
{
    ...
    const float dx = static_cast<float>(gpos.x() - static_cast<qreal>(center.x()));
    const float dy = static_cast<float>(gpos.y() - static_cast<qreal>(center.y()));
    if (dx == 0.0f && dy == 0.0f) {
        event->accept();
        return;
    }
