...
## Pass 41: the window moved - now the hand-off and the box edge do not

SYMPTOM (owner, on device): walking around, the shadows re-flowed / flickered
whenever the SDF box was rebuilt, with a big stall at the same moment.

ROOT CAUSE - two of them, both "the answer depends on where the box sits".
The renderer recenters the box on the camera's chunk at every region change, so
anything keyed to the box moves a whole chunk (32 voxels of world X/Z) with
every crossing.

1. THE HAND-OFF POINT (the dominant one). Pass 40 handed the ray to the 2.5D
   march where it CROSSED the box, and every point beyond that switch re-marched
   from a different place after a recenter. Measured on real terrain
   (probe_rebuild_swap: terrain-surface columns, box centered chunk 0 vs chunk
   1): 806/4239 = 19.0% of the box's outer band columns and 9.5% of the window's
   columns changed visibility by > 0.05 across the crossing. The field's own
   values were NOT the issue - one chunk deeper only 0.2% of columns changed,
   and the halo experiments measured the value band's own contribution at ~0.4%
   (19.0% vs 19.4%).
   FIX: the hand-off is a FIXED distance along the ray now
   (kSdfTraceVoxels = 64 voxels of ray; SdfField::sphereTracedShadowExits, shader
   sunRayEscapesSdf3d), so it is a function of the shaded point and the sun
   alone. A point the box cannot give the fixed term to - a lateral face closer
   than the fixed length (the band along a box face), a start point outside the
   box, or a sun too low to ascend - gets the plain 2.5D march from the shaded
   point, i.e. exactly the VV_SDF_SHADOWS=0 path: the same answer whatever the
   box does. The Y faces are the world floor and sky (the same plane wherever
   the box sits), so a ray that climbs out of the world still hands over there.

2. THE BOX'S EDGE BAND (smaller, but still visible). A chamfer EDT only sees the
   solids inside the box it runs over, so the field's outermost chunk-wide band
   carries truncated distances whose values move with the placement: 5.2% of the
   shared cells (up to 19.5 voxels) and 33.4% of the edge chunk's own cells
   across a one-chunk recenter (probe_rebuild_swap); against a one-chunk-bigger
   reference build over the test world, 30,707 of 589,824 sampled cells differ
   for a halo-less box and 0 for a box built a chunk larger than the region it
   is queried over (testSdfHaloStability). With the fixed hand-off a column
   whose sunward face is closer than the fixed length also has no room for the
   term, so the band was still switching models there.
   FIX: the window is one chunk bigger (kSdfHalfChunks 3 -> 4, i.e. 8x8 chunks).
   Everything the renderer used to shade is then a whole chunk inside the build,
   and the truncated band plus the "no room for the trace" band sit a chunk
   further out where the 2.5D continuation shades anyway. Cost, measured
   (probe_sdf_cost, 2 threads, chunk 32 x 128 x 32): 4.7M -> 8.4M cells, build
   123 -> 230 ms, seed buffer 18 -> 32 MB.

MEASURED AFTER (test output, real terrain, the shipped geometry):

    sdf reshade: 9025 near-field ground columns (7991 hand-offs, 1034 with no
    field term) unchanged by a one-chunk recenter (max hand-off delta 0.0000
    voxels along the ray, max visibility delta 0.00017); the pass-40 crossing
    hand-off would have moved by up to 78.4 voxels
    sdf halo: near field 577600 cells, shipped vs a one-chunk-bigger reference:
    0 mismatches (max 0.000 voxels); the shipped box's own edge chunk:
    2999/454656 (max 12.242 voxels)

The near field is exactly invariant: every hand-off point, distance and
visibility of the 6x6 window's 9,025 ground columns is identical for the box at
chunk 0 and at chunk 1. What is left is the box's own outer chunk - a chunk
beyond the region the renderer shaded before this pass.

TEST: testSdfReshadeInvariance (new: builds the field for the box at chunk 0 and
at chunk 1 from the real chunks and compares every ground column's hand-off in
WORLD space, plus the crossing distances pass-40 would have used),
testSdfHaloStability (retargeted: the shipped box's near field against a
one-chunk-bigger reference), testSdfBoxHandoff (the fallback is exactly the
plain 2.5D march when the box ends before the fixed length; a trace that fits
still hands over and leaks nothing - the 8-voxel trace exercises the same code
path the shipped 64 takes in the renderer's box). Full suite green; the exact
binary path (VV_SDF_SHADOWS=0) is untouched, and the 2.5D fallback call still
passes (0, 1) so its output is bit-identical.

NOT FIXED YET (the measured next steps):
- The rebuild is still synchronous in the frame: the box is built on a worker
  thread, but the frame joins it and the seed upload fence-waits (18 -> 32 MB),
  so a crossing can still hitch ~100-250 ms. That is the "big latency" half of
  the owner's report and the next pass (non-blocking upload, incremental strip
  updates in a world-anchored lattice).
- The window's own edge chunk is placement-dependent by design (it has no data
  outside itself). It now sits 96..128 voxels out; the lever if the owner still
  sees a re-flow at that distance is the same sliding/incremental lattice, which
  is also what would pay for a bigger box.
- LOOK CHANGE: the SDF's soft term now reaches 64 voxels of ray (~26 voxels of
  horizontal travel at this sun) instead of the whole box, and beyond that the
  pass-36 2.5D penumbra takes over. That is the price of a hand-off that cannot
  move; if mid-distance edges look harder than before, the fix is a longer fixed
  trace together with a bigger window.

VERIFIED IN THE SANDBOX (toolchain: scripts/build-linux-toolchain.sh, deps under
/home/user/.cache/vv-deps): pixels_rgba.comp compiles to SPIR-V in the build
(glslangValidator through cmake/Shaders.cmake, exit 0), build/release and
build/debug both build warning-free, ctest green in both (7.4 s / 32.9 s), and
the offscreen smoke run exits cleanly. On-device rendering is still the owner's
gate.
